...
grant all on EMPLEADO.* to 'EMPLEADO'@'%';
use EMPLEADO;
...
#(DA ERROR POR LA FK)SQL Error [1451] [23000]: Cannot delete or update a parent row: a foreign key constraint fails (`EMPLEADO`.`EMPLEADO`, CONSTRAINT `FK_EMPLEADO` FOREIGN KEY (`CODIGO_DEPARTAMENTO`) REFERENCES `DEPARTAMENTO` (`CODIGO`))
